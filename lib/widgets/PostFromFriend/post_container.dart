...
class PostContainer extends StatefulWidget {
  ...
  const PostContainer({Key key, @required this.index_post, @required this.currentUser, this.id_users})
      : super(key: key);

  PostContainerState createState() =>
      PostContainerState(currentUser: currentUser, index_post: index_post, id_users: id_users);
}
...
class PostContainerState extends State<PostContainer> {
  ...
  void initState() {
    super.initState();
    allPosts_combine_allUsers = post_from_friend_controller.GetAllPostFromFriend(currentUser.id_users.toString()); //lấy tất cả post của friend join thông tin users
  }
  ...
  Widget build(BuildContext context) {
    return Card(
      ...
      child: Container(
        ...
        child: FutureBuilder(
            future: allPosts_combine_allUsers,
            //tìm trong bảng users với mỗi id_users tương ứng trong stories
            builder: (context, snapshot) {
              if (snapshot.hasData) {
                print("-------------------index_post = " + snapshot.data[index_post].title.toString());
               // return Container();
                return Column(
                  children: [
                    Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          snapshot.data[index_post].media_url.toString() != null
                    ...
                    snapshot.data[index_post].media_url.toString() != null
                        ? Padding(
                            padding: const EdgeInsets.symmetric(vertical: 8.0),
                            child: CachedNetworkImage(imageUrl: snapshot.data[index_post].media_url.toString()),
                          )
                        : const SizedBox.shrink(),
                    Padding(
                      padding: const EdgeInsets.symmetric(horizontal: 12.0),
